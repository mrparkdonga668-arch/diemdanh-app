(function() {
    ...
    function completeAttendance(session) {
        ...
        const now = getNow();
        // Tạo chữ ký (signature) để vượt qua Firebase Rules
        // Chữ ký này dựa trên tổ hợp thông tin và SECRET_KEY
        const signature = CryptoJS.HmacSHA256(`${STUDENT_ID}_${activeClassId}_${now}`, SECRET_KEY).toString();
        
        fetch(`${FB_URL}/checkins.json`, { 
            method: "POST", 
            body: JSON.stringify({ student_id: STUDENT_ID, class_id: activeClassId, timestamp: now, signature: signature }) 
        }).then(res => {
            if (res.ok) {
                statusDiv.innerHTML = `<h3 style="color:green;">🎉 ĐIỂM DANH THÀNH CÔNG!</h3>`;
                activateRelayMode(session);
            } else {
                handleFailure("Lỗi xác thực");
                isSubmitting = false;
            }
        }).catch(err => {
            handleFailure("Lỗi mạng");
            isSubmitting = false;
        });
    }
        ...
        try {
            ...
            navigator.geolocation.getCurrentPosition((pos) => {
                if (getDist(pos.coords.latitude, pos.coords.longitude, targetSession.lat, targetSession.lon) > 0.3) {
                    statusDiv.innerHTML = "❌ Không ở trong lớp học!"; btnStart.style.display = "inline-block";
                } else {
                ...
                statusDiv.innerHTML = "🚫 Cần cho phép GPS!"; btnStart.style.display = "inline-block"; 
